...
# COM port setting
COM_PORT = "/dev/cu.usbmodem00023716951"
...
class AnyCloud:
  ...
  def serial_receive(self):
    ...
    if read_val != b'':
      ...
      if read_val == b'>': #LUC change '>' for '\r'
        #print('LUC ',read_val)
  ...
  def evt_read_certificate(self) :
    ...
    for i in cert_list:
      cert = cert + i +'\r\n'
    #print(cert)
    f = open("WFI32_DevCert.cer", "w")
    f.write(cert)
    f.close()

    #openssl x509 -in AnyCloud.PEM -text
    cmd_line = "openssl x509 -in WFI32_DevCert.cer -text"
    ...
    certName = "WFI32_DevCert_"+cn+".cer"
    f = open(certName, "w")
    f.write(cert)
    f.close()
    ...
    #if os.name == 'posix' :
    #  cmd_line = "mv WFI32_DevCert " + cn + ".cer"
    #else:
    #  cmd_line = "rename WFI32_DevCert " + cn + ".cer"
    #print(cmd_line)
    #p = subprocess.Popen(cmd_line, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    #for line in p.stdout.readlines():
    #  print(line.rstrip().decode())

    print("\r\nPlease create a Thing \"" + cn + "\" into AWS IoT using the Common Name (CN) of this certificate")
    #print("For the connection parameters, select individual enrollement -> x.509 Certificate")
    print("Upload \""+certName+"\"  certificate\r\n")
...
parser = ArgumentParser(description='Create a AWS IoT Certificate from WFI32 T&G device')
parser.add_argument("-p", "--port", dest="serial_port",help="which serial port to open")
...
ac = AnyCloud(args.serial_port, args.baudrate, False)
